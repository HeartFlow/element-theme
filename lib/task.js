...
exports.build = function (opts) {
  ...
  var cssFiles = '*'
  var customCssFiles = '*'
  var customFilesFolder = 'custom'
  var customFolder = 'theme-in/' + customFilesFolder + '/'
  ...
  var customPath = path.resolve(config.themePath, './' + customFilesFolder + '/' + customCssFiles + '.css')
  ...
  stream = gulp.src([opts.config || config.config, componentsPath, customPath])
      ...
      .pipe(gulp.dest(function(file) {
        if (file.path.indexOf(customFolder) > -1) {
          return (opts.out || config.out) + '/' + customFilesFolder
        }
